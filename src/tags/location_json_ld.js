...
const location_json_ld = {
  ...
  render: async function (ctx, hash) {
    ...
    return JSON.stringify({
      ...
      'description': formatLocationDescription(
        location,
        this.params['description'],
        engine
      ),
      'url': formatLocationUrl(
        location,
        this.params['template_key'] || 'store-page',
        engine
      ),
...
function formatLocationDescription(location, description, engine) {
  ...
  return engine.parseAndRenderSync(
    `{{ "${desc}" | newline_to_br | strip_newlines | replace: '<br /><br />', ' ' | replace: '<br />', ' ' | strip_html | escape | strip }}`
  )
}
